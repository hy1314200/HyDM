...
namespace Skyline.Frame
{
  ...
  /// </summary>
    partial class EsriLicenseInitializer
  {
    /// <summary>
    ...
    public string LicenseMessage()
    {
      ...
      {
        prodStatus = MessageNoLicensesRequested + System.Environment.NewLine;
      }
          ...
          || m_productStatus.ContainsValue(esriLicenseStatus.esriLicenseCheckedOut))
      {
        prodStatus = ReportInformation(m_AoInit as ILicenseInformation,
            ...
            esriLicenseStatus.esriLicenseCheckedOut) + System.Environment.NewLine;
      }
      else
      {
        //Failed...
        ...
        {
          prodStatus += ReportInformation(m_AoInit as ILicenseInformation,
              item.Key, item.Value) + System.Environment.NewLine;
        }
      }
      ...
      foreach (KeyValuePair<esriLicenseExtensionCode, esriLicenseStatus> item in m_extensionStatus)
      {
        ...
        if (!string.IsNullOrEmpty(info))
            extStatus += info + System.Environment.NewLine;
      }
    ...
    private const string MessageNoRuntimeBinding = "错误的ArcGIS运行时绑定。";
    private const string MessageNoAoInitialize = "ArcGIS组件初始化失败。";
    private const string MessageNoLicensesRequested = "产品：没有许可要求。 Product: No licenses were requested";
    private const string MessageProductAvailable = "产品: {0}: 可用";
    private const string MessageProductNotLicensed = "产品: {0}: 没有许可";
    private const string MessageExtensionAvailable = " 扩展: {0}: 可用";
    private const string MessageExtensionNotLicensed = " 扩展: {0}: 没有许可";
    private const string MessageExtensionFailed = " 扩展: {0}: 失败";
    private const string MessageExtensionUnavailable = " 扩展: {0}: 不可用";
