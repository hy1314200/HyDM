...
#Region "公有函数"
    ...
    Public Function ConvertVctFileToSDE(ByVal p_Layer As VctLayer, ByVal p_Workspace As IWorkspace, ByVal p_LayerName As String, ByVal p_DatasetName As String, ByVal p_SpatialReference As ISpatialReference, ByRef p_ErrMessage As String) As Boolean
        ...
        Try
            Dim mMapDBClient As New EsriHelper
...
#Region "私有函数"
    ...
    Private Sub ReadLayer(ByVal p_Line As String)
        ...
        If mValues IsNot Nothing Then
            ...
            Select Case mValues(2).Trim.ToUpper
                Case "POINT"
                    mVctLayer.LayerType = enumLayerType.Point
                Case "LINE"
                    mVctLayer.LayerType = enumLayerType.Line
                Case "POLYGON"
                    mVctLayer.LayerType = enumLayerType.Polygon
                Case "ANNOTATION"
                    mVctLayer.LayerType = enumLayerType.Annotation
                Case "IMAGE"
                    mVctLayer.LayerType = enumLayerType.Image
            End Select
                ...
                If mLayer IsNot Nothing Then
                    ...
                    Select Case mValues(1).ToUpper
                        Case "INTEGER", "INT"
                            ...
                            mField.Type = enumFieldType.VctInt
                        Case "CHAR", "VARCHAR"
                            Dim mColumn As DataColumn = New DataColumn
                            mColumn.ColumnName = mField.Name
                            mColumn.Caption = mField.Name
                            ...
                            mField.Type = enumFieldType.VctString
                        Case "FLOAT"
                            Dim mColumn As DataColumn = New DataColumn
                            mColumn.ColumnName = mField.Name
                            mColumn.Caption = mField.Name
                            ...
                            mField.Type = enumFieldType.VctDouble
                        Case "DATE", "TIME"
                            Dim mColumn As DataColumn = New DataColumn
                            mColumn.ColumnName = mField.Name
                            mColumn.Caption = mField.Name
                            ...
                            mField.Type = enumFieldType.VctDateTime
                        Case "VARBIN"
                            Dim mColumn As DataColumn = New DataColumn
                            mColumn.ColumnName = mField.Name
                            mColumn.Caption = mField.Name
                            ...
                            mField.Type = enumFieldType.VctVarbin
                        Case "BOOLEAN", "BOOL"
                            Dim mColumn As DataColumn = New DataColumn
                            mColumn.ColumnName = mField.Name
                            mColumn.Caption = mField.Name
                            ...
                            mField.Type = enumFieldType.VctBoolean
                    End Select
    ...
    Private Sub ReadPolygon(ByVal p_Line As String)
        Select Case m_Count
            ...
            Case 3
                ...
                'End If
                If Me.m_Polygon IsNot Nothing Then
                    Me.m_Polygon.FeatureType = Convert.ToInt32(p_Line.Trim)
                End If
                Me.m_Count += 1
            Case 4
                If Me.m_Polygon IsNot Nothing Then
            ...
            Case 5 To m_PointCount + 5
                ...
                If Me.m_Polygon IsNot Nothing Then
                    ' 获取环号
                    Dim ringID As Integer = 0
                    Dim indexOffset As Integer = -1
                    '张航宇，只有当定义了线号时才读取线号，即数组长度大于定义的维数时
                    If (mValues.Length > Me.Header.Dimension) Then
                        indexOffset = 0
                        ringID = Convert.ToInt32(mValues(0))
                    End If

                    If (m_Polygon.PointCollections.Count < ringID + 1) Then
                        m_Polygon.PointCount.Add(m_PointCount)
                        m_Polygon.PointCollections.Add(New VctPointCollection)
                    End If

                    Dim pVertex As VctPoint = New VctPoint
                    pVertex.X = Convert.ToDouble(mValues(indexOffset + 1))
                    pVertex.Y = Convert.ToDouble(mValues(indexOffset + 2))
                    m_Polygon.PointCollections(ringID).Points.Add(pVertex)
                ...
                '张航宇(读取完数据继续下一条记录)
                If m_Count = m_PointCount + 5 Then
                    Me.m_Count = 0
                ...
                '        '张航宇，只有当定义了线号时才读取线号，即数组长度大于定义的维数时
                '        Dim mLineID As Integer = 0
                '        If (mValues.Length > Me.Header.Dimension) Then
                '            mLineID = Convert.ToInt32(mValues(i))
                '        End If
                ...
                'If Me.m_Count = m_PointCount + 4 Then
                '    Me.m_Count = 0
                'End If
        End Select
    ...
    Private Sub SaveAttributeOfFeature(ByVal p_Row As DataRow, ByVal p_Feature As IFeature)
        ' 张航宇，记录可能不存在
        If (p_Row Is Nothing Or p_Feature Is Nothing) Then
            Return
        End If
    ...
    Private Function GetAttrFromAttributeTable(ByVal p_ID As Integer, ByVal p_IDFieldName As String, ByVal p_AttrTable As DataTable) As DataRow
        ...
        '张航宇，若记录未定
        If mRow Is Nothing Or mRow.Length = 0 Then
            Return Nothing
        End If
        Return mRow(0)
    End Function
    ...
    Private Function GetVctOutFeatureClass(ByVal p_FeatureClass As IFeatureClass, ByVal p_OutName As String, ByRef p_FID As Integer, ByRef p_ID As Int64) As VctOutFeatureClass
        ...
        mVctField.Name = "ID"
        mVctField.Type = enumFieldType.VctInt
        mVctField.Length = 10
                ...
                Case Else
                    ...
                    Select Case mField.Type
                        Case esriFieldType.esriFieldTypeBlob
                            mVctField1.Type = enumFieldType.VctVarbin
                        ...
                        Case esriFieldType.esriFieldTypeDate
                            mVctField1.Type = enumFieldType.VctDateTime
                            mVctField1.Length = mField.Length
                            mVctField1.DecimalLength = mField.Precision
                        ...
                        Case esriFieldType.esriFieldTypeDouble, esriFieldType.esriFieldTypeSingle
                            mVctField1.Type = enumFieldType.VctDouble
                            mVctField1.Length = mField.Length
                            mVctField1.DecimalLength = mField.Precision
                        ...
                        Case esriFieldType.esriFieldTypeInteger, esriFieldType.esriFieldTypeSmallInteger
                            mVctField1.Type = enumFieldType.VctInt
                            mVctField1.Length = mField.Length
                            mVctField1.DecimalLength = mField.Precision
                        ...
                        Case esriFieldType.esriFieldTypeString, esriFieldType.esriFieldTypeXML
                            mVctField1.Type = enumFieldType.VctString
                            mVctField1.Length = mField.Length
                            mVctField1.DecimalLength = mField.Precision
        ...
        Dim mLayerType As enumEsriLayerType = Me.GetLayerType(p_FeatureClass)

        Select Case mLayerType
            Case enumEsriLayerType.Point
                mVctFeatureClass.GeometryType = enumLayerType.Point
            Case enumEsriLayerType.Line
                mVctFeatureClass.GeometryType = enumLayerType.Line
            Case enumEsriLayerType.Polygon
                mVctFeatureClass.GeometryType = enumLayerType.Polygon
            Case enumEsriLayerType.Raster
                mVctFeatureClass.GeometryType = enumLayerType.Image
            Case enumEsriLayerType.Annotation
                mVctFeatureClass.GeometryType = enumLayerType.Annotation
        End Select
    ...
    Private Sub WriteTableStructure(ByVal p_StreamWriter As StreamWriter, ByVal p_FeatureClassList As List(Of VctOutFeatureClass))
        ...
        For i As Integer = 0 To p_FeatureClassList.Count - 1
            ...
            For j As Integer = 0 To mFeatureClass.OutFields.Count - 1
                ...
                If mField.Type = enumFieldType.VctDouble Then
    ...
    Private Function GetLayerType(ByVal p_FeatureClass As IFeatureClass) As enumEsriLayerType
        '栅格
        If p_FeatureClass.FeatureType = esriFeatureType.esriFTRasterCatalogItem Then
            ...
            If mRasterCatalog.IsRasterDataset Then
                Return enumEsriLayerType.Raster
            Else
                Return enumEsriLayerType.RasterCatalog
            End If
        End If
        '注记
        If p_FeatureClass.FeatureType = esriFeatureType.esriFTAnnotation Or _
            ...
            Return enumEsriLayerType.Annotation
        End If
        '点
        If p_FeatureClass.ShapeType = esriGeometryType.esriGeometryPoint Or _
            ...
            Return enumEsriLayerType.Point
        End If
        '线
        ...
        If p_FeatureClass.ShapeType = esriGeometryType.esriGeometryPolygon Or _
            ...
            Return enumEsriLayerType.Polygon
        End If
        Return enumEsriLayerType.UnKnown
    End Function
