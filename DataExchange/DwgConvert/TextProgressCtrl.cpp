...
// file may be reHyributed by any means PROVIDING it is not sold for
...
void CTextProgressCtrl::DoEvents()
{
	MSG message;
	for (int i = 0; i < 10; i++)
	{
...
LRESULT CTextProgressCtrl::OnSetText(WPARAM, LPARAM lParam)//UINT, LPCTSTR szText)
{
...
{
	int iRet = SetPos(m_nPos + m_nStepSize);
	DoEvents();
	return iRet;
}
...
void CTextProgressCtrl::SetText(CString sText)
{
