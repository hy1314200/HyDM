...
std::string* CXMLFile::ParseKeys(const char* cstrFullKeyPath, int &iNumKeys)
{
	...
	if (pCStrKeys)
	{
		...
		while (iFind != -1)
		{
			iFind = (int)strFullKeyPath.find('/', iLastFind);
			if (iFind > -1)
			{
				iCount++;
