                ...
                if (m_bFoundTableStructureBegin == true)
                {
                    ...
                    else
                    {
                        ...
                        if (arrTemp.LongLength >= 2)
                        {
                            ...
                            {
                                Logger.WriteLog("获取VCT属性表【"+tableStructureNode.TableName+"】的字段个数出现异常，请确认该数据是否符合标准！");
                                return null;
                            }
                            if (arrTemp.Length > 2)
                            {
            ...
            if (this.m_streamReader != null)
            {
                ...
                if (m_bFoundPointBegin == true)
                {
                    ...
                    else
                    {
                        ...
                        {
                            Logger.WriteLog("获取点实体的标识码失败，请检查数据是否符合标准！");
                            Logger.WriteLog("读取错误的所在行数【" + m_nReadLineCount + "】");
                            return null;
                        }
                        //再获取要素编码
                        ...
                        {
                            Logger.WriteLog("获取点实体的特征类型失败，请检查数据是否符合标准！");
                            Logger.WriteLog("读取错误的所在行数【" + m_nReadLineCount + "】");
                            return null;
                        }
                        //获取点的个数
            ...
            if (this.m_streamReader != null)
            {
                ...
                if (m_bFoundLineBegin == true)
                {
                    ...
                    else
                    {
                        ...
                        {
                            Logger.WriteLog("获取线实体的标识码失败，请检查数据是否符合标准！");
                            Logger.WriteLog("读取错误的所在行数【" + m_nReadLineCount + "】");
                            return null;
                        }
                        //再获取要素编码
                        ...
                        {
                            Logger.WriteLog("获取线实体的特征类型失败，请检查数据是否符合标准！");
                            Logger.WriteLog("读取错误的所在行数【"+m_nReadLineCount+"】");
                            return null;
                        }
                        //获取线段的条数
                        ...
                        {
                            Logger.WriteLog("获取线实体的节点个数失败，请检查数据是否符合标准！");
                            Logger.WriteLog("读取错误的所在行数【" + m_nReadLineCount + "】");
                            return null;
                        }
                        ...
                        for (int i = 0; i < nSegmentCount; i++)
                        {
                            ...
                            {
                                Logger.WriteLog("获取折线的节点个数失败，请检查数据是否符合标准！");
                                Logger.WriteLog("读取错误的所在行数【" + m_nReadLineCount + "】");
                                return null;
                            }
            ...
            if (this.m_streamReader != null)
            {
                ...
                if (m_bFoundPolygonBegin == true)
                {
                    ...
                    else
                    {
                        ...
                        {
                            Logger.WriteLog("获取面实体的标识码失败，请检查数据是否符合标准！");
                            Logger.WriteLog("读取错误的所在行数【" + m_nReadLineCount + "】");
                            return null;
                        }
                        //再获取要素编码
                        ...
                        {
                            Logger.WriteLog("获取面实体的特性类型失败，请检查数据是否符合标准！");
                            Logger.WriteLog("读取错误的所在行数【" + m_nReadLineCount + "】");
                            return null;
                        }
                        //获取面的标识点坐标
                        ...
                        {
                            Logger.WriteLog("获取面实体的线对象项数失败，请检查数据是否符合标准！");
                            Logger.WriteLog("读取错误的所在行数【" + m_nReadLineCount + "】");
                            return null;
                        }
            ...
            if (this.m_streamReader != null)
            {
                ...
                if (m_bFoundAnnotationBegin == true)
                {
                    ...
                    else
                    {
                        ...
                        {
                            Logger.WriteLog("获取注记实体的标识码失败，请检查数据是否符合标准！");
                            Logger.WriteLog("读取错误的所在行数【" + m_nReadLineCount + "】");
                            return null;
                        }
                        //再获取要素编码
                        ...
                        {
                            Logger.WriteLog("获取注记实体的特征类型失败，请检查数据是否符合标准！");
                            Logger.WriteLog("读取错误的所在行数【" + m_nReadLineCount + "】");
                            return null;
                        }
                        //获取注记的内容
            ...
            if (this.m_streamReader != null)
            {
                ...
                if (m_bFoundTableBegin == true)
                {
                    ...
                    else
                    {
                        ...
                        {
                            Logger.WriteLog("构造数据表记录错误，导致错误所在的文件行数【" + m_nReadLineCount + "】");
                            return null;
                        }
                        return pRecordNode;
                    }
